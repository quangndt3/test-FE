...
const items = [
  getItem(<Link to='/manage'>Sản phẩm</Link>, '1', <PieChartOutlined />),
...
const AdminLayout = () => {
  ...
  useEffect(() => {
    ...
    if(user?.role!='admin'){
      message.error("Bạn không được phép truy cập vào địa chỉ này")
  navigate("/")
    }
  }, [])
  const logout = ()=>{
  ...
  return <>
    <Layout style={{ minHeight: '100vh' }}>
      ...
      <Layout>
        <Header className='flex justify-end items-center' style={{ padding: 0, background: colorBgContainer }} >
        <span className='text-[14px] mr-[10px]'>Chào <strong>{user?.email}</strong></span>
          <div className='!w-[100px] '>
            ...
            </>} > <div className='max-sm:hidden header-icon-item header-search-icon text-[20px] ml-[30px] transition-colors duration-300 cursor-pointer hover:text-[#d2401e]'>
                <Popover
                  placement='bottom'
                  trigger='hover'
                >
