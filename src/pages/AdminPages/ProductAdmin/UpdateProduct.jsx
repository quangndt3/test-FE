...
import { Form, Space, Input, Radio, Divider, message } from 'antd';
...
const UpdateProduct = () => {
   ...
   const handleGetFiles = (files, public_id) => {
      if (!public_id) {
   ...
   useEffect(() => {
      ...
      data?.images?.forEach((img) => {
         const file = { uid: img.public_id, url: img.url, name: 'images', status: 'done' };
   ...
   const handleSubmit = async () => {
      try {
         ...
         await updateProduct({ idProduct: id, ...{ ...newFormData, productName } }).then(res=>{
            res
